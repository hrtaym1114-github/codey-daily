...
FEATURES = [
    ...
    },
    {
        ...
        "links": [], "difficulty": 2, "tier": "free", "related": ["file-settings-json", "tool-bash"],
    },
    {
        "id": "slash-ide", "name": "/ide", "category": "slash-command",
        "summary_ja": "IDE拡張機能を自動インストール",
        "description_ja": "VS Code や JetBrains などの IDE 拡張機能をコマンドから自動インストールする。IDE 内で Claude Code を利用するための拡張機能のオンボーディングを自動化。インストール処理のエラーは v2.1.73 で修正済み。",
        "examples": [
    ...
    {
        "id": "slash-plugins", "name": "/plugins", "category": "slash-command",
        "summary_ja": "プラグイン一覧とマーケットプレイス探索ビューを開く",
        "description_ja": "インストール済みプラグインの Installed タブとマーケットプレイスから新プラグインを探す discover ビューを持つプラグインブラウザ。v2.1.2 で Installed タブがプラグインと MCP をスコープ別グループ化に統合（v2.0.74 で discover ビューの表示バグ修正）。管理操作は /plugin も参照。",
        "examples": [
            {"title": "プラグイン一覧", "code": "/plugins"},
            {"title": "探索モード", "code": "/plugins discover"},
        ],
        "links": [], "difficulty": 2, "tier": "free", "related": ["slash-plugin", "slash-reload-plugins", "skill-marketplace"],
    },
    {
        "id": "slash-background", "name": "/background", "category": "slash-command",
        "summary_ja": "バックグラウンドセッションを起動（/bg のフルネーム）",
        "description_ja": "/bg コマンドのフルネーム。バックグラウンドでエージェントセッションを起動する。v2.1.147 でスキルやカスタムスラッシュコマンドのみが入力された場合にセッションを拒否するバグが修正された。実行中タスクは /tasks で確認可能。",
        "examples": [
            {"title": "バックグラウンド起動", "code": "/background"},
        ],
        "links": [], "difficulty": 2, "tier": "free", "related": ["slash-bg", "slash-tasks"],
    },
    {
        "id": "slash-workflows", "name": "/workflows", "category": "slash-command",
        "summary_ja": "ダイナミックワークフローの実行一覧を管理",
        "description_ja": "バックグラウンドで数十〜数百のエージェントを並列オーケストレーションするダイナミックワークフローを管理するコマンド（v2.1.154追加）。Claudeにワークフロー作成を依頼した後、/workflows で実行中・完了一覧を確認できる。大規模・複雑なタスクの実行管理に使う。",
        "examples": [
            {"title": "ワークフロー一覧確認", "code": "/workflows"},
        ],
        "links": [], "difficulty": 3, "tier": "free", "related": ["slash-tasks", "slash-agents", "slash-bg"],
    },
    {
        "id": "slash-logout", "name": "/logout", "category": "slash-command",
        "summary_ja": "claude agentsビューでサインアウト",
        "description_ja": "claude agents（エージェントビュー）セッション内でサインアウトするコマンド（v2.1.154追加）。以前はバックグラウンドセッションに誤送信されていたが修正済み。通常の対話セッションで認証をやり直す場合は /login を使う。",
        "examples": [
            {"title": "サインアウト", "code": "/logout"},
        ],
        "links": [], "difficulty": 1, "tier": "free", "related": ["slash-login"],
    },
    {
        "id": "slash-settings", "name": "/settings", "category": "slash-command",
        "summary_ja": "/config のエイリアス（設定表示・変更）",
        "description_ja": "設定を表示・変更する /config コマンドのエイリアス（v2.0.71で追加）。テーマ、モデル、その他のユーザー設定を変更する際に /settings と入力しても動作する。config とは別に /settings という名称で設定操作に慣れているユーザー向けの互換性コマンド。",
        "examples": [
            {"title": "設定表示", "code": "/settings"},
        ],
        "links": [], "difficulty": 1, "tier": "free", "related": ["slash-config", "file-settings-json"],
    },
    {
        "id": "slash-proactive", "name": "/proactive", "category": "slash-command",
        "summary_ja": "/loop のエイリアス（定期プロンプト実行）",
        "description_ja": "プロンプトを定期実行する /loop コマンドのエイリアス（v2.1.105で追加）。/loop と同様に指定した間隔でプロンプトやスラッシュコマンドを繰り返し実行できる。Bedrock/Vertex でも利用可。新しい記法では /loop を使用することを推奨。",
        "examples": [
    ...
    {
        "id": "slash-desktop", "name": "/desktop", "category": "slash-command",
        "summary_ja": "Claude Codeデスクトップアプリとの接続を管理",
        "description_ja": "Claude Code for Desktop（デスクトップアプリ）との接続を設定・管理するコマンド。/install-github-app・/resume・/web-setup と同様Escでダイアログを閉じられる（v2.1.136）。worktreeに出入りした後に作業ディレクトリを特定できず失敗する不具合がv2.1.198で修正された。",
        "examples": [
            {"title": "デスクトップアプリ接続", "code": "/desktop"},
        ],
        "links": [], "difficulty": 2, "tier": "free", "related": ["slash-remote-control", "slash-web-setup"],
    },
    {
        "id": "slash-vim", "name": "/vim", "category": "slash-command",
        "summary_ja": "vimキーバインドを有効化する旧コマンド（廃止済み）",
        "description_ja": "テキスト入力にvim風キーバインドを有効化する旧コマンド（v0.2.34追加）。v2.1.92で削除され、以降は /config のEditor modeからvimモードを切り替える。",
        "examples": [
    ...
    {
        "id": "slash-tag", "name": "/tag", "category": "slash-command",
        "summary_ja": "セッションにタグを付ける旧コマンド（廃止済み）",
        "description_ja": "セッションにタグを付与する旧コマンド。/rename と同様、git worktree等の別ディレクトリからセッションを再開した際に誤ったセッションを更新してしまう不具合があった（v2.1.19修正）。v2.1.92で削除された。",
        "examples": [
            {"title": "タグ付け（廃止済み）", "code": "/tag"},
        ],
        "links": [], "difficulty": 1, "tier": "free", "related": ["slash-rename"],
    },
    {
        "id": "slash-allowed-tools", "name": "/allowed-tools", "category": "slash-command",
        "summary_ja": "ツール許可管理コマンドの旧名（現 /permissions）",
        "description_ja": "ツールの許可・拒否ルールを管理する旧コマンド名。v1.0.7で /permissions にリネームされた。許可・拒否履歴の確認やリトライは現在の /permissions コマンドを使用する。",
        "examples": [
            {"title": "許可管理（旧コマンド名）", "code": "/allowed-tools"},
        ],
        "links": [], "difficulty": 1, "tier": "free", "related": ["slash-permissions"],
    },
    {
        "id": "slash-approved-tools", "name": "/approved-tools", "category": "slash-command",
        "summary_ja": "ツール権限管理の最初期コマンド（廃止済み）",
        "description_ja": "ツール権限を管理するための最初期のコマンド（v0.2.26追加、「New /approved-tools command for managing tool permissions」）。ツール許可管理の仕組みはその後 /allowed-tools（v1.0.7で /permissions にリネーム）へと発展し、現在は /permissions が使われている。",
        "examples": [
    ...
    {
        "id": "cli-effort", "name": "--effort", "category": "cli",
        "summary_ja": "起動時のThinking処理強度を指定",
        "description_ja": "low/normal/high/max/auto の処理強度を起動時に固定する。Opus モデルでは xhigh も指定可能（v2.1.111追加）。`--effort` 設定は `claude agents` やバックグラウンドセッション（/bg）にも引き継がれる（v2.1.142）。起動後は /effort コマンドでもセッション内変更可能。",
        "examples": [
            {"title": "最大思考量で起動", "code": "claude --effort max"},
            {"title": "Opus専用xhigh", "code": "claude --effort xhigh"},
        ],
        "links": [], "difficulty": 2, "tier": "free", "related": ["slash-effort", "slash-model"],
    },
    {
        "id": "cli-permission-mode", "name": "--permission-mode", "category": "cli",
        "summary_ja": "ツール許可モードを起動時に設定",
        "description_ja": "default/acceptEdits/bypassPermissions/plan の権限モードを起動時に指定する（v2.1.132修正）。`claude agents` での起動や `-p` ヘッドレスモードでも有効。`--resume`/`-c` 再開時は前回の設定を引き継がないため都度指定が必要。`claude agents` フラグとしても使用可能（v2.1.142）。",
        "examples": [
            {"title": "編集自動許可モード", "code": "claude --permission-mode acceptEdits"},
            {"title": "完全自動モード", "code": "claude --permission-mode bypassPermissions"},
        ],
        "links": [], "difficulty": 2, "tier": "free", "related": ["mode-permission", "cli-dangerously-skip-permissions"],
    },
    {
        "id": "cli-fallback-model", "name": "--fallback-model", "category": "cli",
        "summary_ja": "プライマリモデル過負荷時の代替モデルを設定",
        "description_ja": "プライマリモデルが過負荷または利用不可の場合に順に試す代替モデルを指定（v2.1.166追加）。インタラクティブセッションでも動作し、`fallbackModel` settings.json 設定で最大3つを永続設定可能。compaction でもフォールバック設定が引き継がれる（v2.1.178修正）。",
        "examples": [
            {"title": "フォールバックモデル指定", "code": "claude --fallback-model claude-sonnet-4-6"},
        ],
        "links": [], "difficulty": 2, "tier": "free", "related": ["cli-model", "file-settings-json"],
    },
    {
        "id": "cli-settings", "name": "--settings", "category": "cli",
        "summary_ja": "外部JSONファイルからsettingsを読み込む",
        "description_ja": "JSONファイルで設定を指定して ~/.claude/settings.json を補完・上書きする（v1.0.61追加）。`/bg` バックグラウンドセッションと `claude agents` にも引き継がれる（v2.1.142）。`--bare` フラグとの組み合わせで API キーやエージェント設定のファイル化にも使う。",
        "examples": [
            {"title": "外部設定ファイル指定", "code": "claude --settings ./team-settings.json"},
            {"title": "bareモードと組み合わせ", "code": "claude --bare --settings ./ci.json -p \"テスト実行して\""},
        ],
        "links": [], "difficulty": 3, "tier": "free", "related": ["file-settings-json", "cli-bare"],
    },
    {
        "id": "cli-strict-mcp-config", "name": "--strict-mcp-config", "category": "cli",
        "summary_ja": "サブエージェントにもMCP設定制限を適用",
        "description_ja": "サブエージェント（Agent tool）にも MCP 設定制限を適用し、設定外 MCP サーバーへのアクセスに警告を表示する（v2.1.153追加）。`/bg` バックグラウンドセッションにも引き継がれる（v2.1.143）。エンタープライズ環境で MCP アクセスを制御する際に使う。",
        "examples": [
